...
export class MultimodalLiveClient extends EventEmitter<MultimodalLiveClientEventTypes> {
  ...
  protected async receive(blob: Blob) {
    ...
    } else if (typeof (response as any).type === "string") {
      // Emit any message with a 'type' field as an adkevent
      // This allows handling custom domain events without monkey-patching receive()
      this.emit("adkevent", response as any);
    } else {
  ...
  sendRealtimeInput(chunks: GenerativeContentBlob[]) {
    ...
    // Track sent chunks but DON'T discard them. 
    // Discarding audio chunks causes vocal "breaking" / stuttering.
    if (hasAudio && !hasVideo) {
    ...
    for (const chunk of chunks) {
      ...
      // For remote mode: wrap first content in {user_id, session_id, live_request} format
      if (!this.firstContentSent) {
        data = {
          user_id: this.userId || "default_user",
          session_id: this.projectId,
          live_request: data,
        };
