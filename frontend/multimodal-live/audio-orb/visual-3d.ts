...
export class GdmLiveAudioVisuals3D extends LitElement {
  ...
  private init() {
    ...
    const containerHeight = this.canvas.clientHeight || window.innerHeight;
    ...
    new EXRLoader().load('/piz_compressed.exr', (texture: THREE.Texture) => {
      ...
      console.error('❌ [Visual3D] Error loading EXR texture:', err);
    });
  ...
  private animation() {
    ...
    // Only update analysers if they exist (audio nodes have been set)
    ...
    if (sphereMaterial.userData.shader && this.inputAnalyser && this.outputAnalyser) {
